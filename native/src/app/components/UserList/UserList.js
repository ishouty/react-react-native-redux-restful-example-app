...
class UserList extends React.Component {
  ...
  _renderItem = ({ item, index }) => {
    ...
    let newStyles = isEven(index + 1)
      ? [styles.item, styles.gray]
      : [styles.item, styles.white]

    return (
      <View style={newStyles}>
        <View
          style={
            imageDisplay === 'small' ? { flexDirection: 'row' } : {}
          }
        >
          <ProfileAvatar
            sourceImage={{ uri: item.picture.large }}
            size={this.props.imageDisplay}
          />
          {item.name && (
            <Text style={[{ marginLeft: 5 }, styles.summaryItem]}>
              {capitalizedText(item.name.title)}{' '}
              {capitalizedText(item.name.first)}{' '}
              {capitalizedText(item.name.last)}
            </Text>
          )}
        </View>
        ...
        {item.gender && (
          <Text style={styles.titleText}>
            {CommonText.gender} {capitalizedText(item.gender)}
          </Text>
        )}
        {item.email && (
          <Text style={styles.titleText}>
            {CommonText.email} {item.email}
          </Text>
        )}
        {item.dob.age && (
          <Text style={styles.titleText}>
            {CommonText.age} {item.dob.age}
          </Text>
        )}

        {item.phone && (
          <Button
            title={CommonText.call}
            bsClass={{ button: styles.buttonContact }}
            callback={() => {
