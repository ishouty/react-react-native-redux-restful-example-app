    ...
    return (next) => (action) => {
        ...
        if (action.type == '@@router/LOCATION_CHANGE') {
            ...
            if (getState().navigation.previousRoute.length > 0) {
                ...
                if (typeof getState().navigation.previousRoute[route - 1] !== 'undefined') {
                    mixpanel.track(MIX_PANEL.pageView, {page: getState().navigation.previousRoute[route - 1]})
                }

            }

            mixpanel.track(MIX_PANEL.pageEnter, {page: action.payload.pathname})
            return result
