...
class ResetPassword extends Component {
    ...
    resetState() {
        this.props.dispatch({type: actionTypes.RESET_PASSWORD, payload: {completeForm: false, email: null}})
    }

    BackToResetPassword(dispatch) {
        dispatch({type: actionTypes.RESET_PASSWORD, payload: {completeForm: false, email: null}})
    }
        ...
        return (
            <div className="reset-password-container">
                ...
                <div>
                    ...
                    { resetPassword.completeResetPassword.completeForm == false ?
                        ...
                        <Row className="reset-password-check-mail">
                            <Row style={ { textAlign : 'center'} }>
                                ...
                                <Row className="firstMessage">
                                    <span>{ResetPasswordCheckEmailText.description[0] }
                                    <b>{resetPassword.completeResetPassword.email}</b>
                                    </span>
                                </Row>

                                <Row className="spacing">
                                    <span>{ResetPasswordCheckEmailText.description[1]}</span>
                                </Row>

                                <Row>
                                    <span>{ResetPasswordCheckEmailText.description[2]}</span>
                                </Row>

                                <Row className="spacing">
                                    <span>{ResetPasswordCheckEmailText.description[3]}</span>
                                </Row>

                                <Row className="spacing">
                                    <a onClick={()=> { this.BackToResetPassword(dispatch) }}>{ ResetPasswordCheckEmailText.link } </a>
                                </Row>
