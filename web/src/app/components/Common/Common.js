...
class BannerRightSection extends Component {
    ...
    render () {
        ...
        return (
            <Col sm={6} className="right-section" style={styles.rightSection}>
...
const renderFieldText = ({ id, input, label, type, disabled, placeholder, helpBlock, userValue, meta: { touched, error, warning } }) => (
...
class LoadingButton extends Component {
    ...
    render() {
        ...
        const showLoading = loading.loading.common && (show === undefined || show)

        return (
            <div className="loading-btn-container">
                { showLoading ? <CircularProgress color="#000" size={50} thickness={3}/> : null }
            </div>
        )
    }
            ...
            <div>
                {
                    (banner.fail || banner.success) ?
                        <div className={`banner ${className} ${bsClass}`}
                             onClick={(dispatch)=> { this.close(dispatch) } }>
