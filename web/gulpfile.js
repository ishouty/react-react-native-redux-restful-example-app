...
gulp.task("default", ["dev-server:dev:hmr"])

gulp.task('build-dev', ['clean-directory', 'webpack:build:dev', "server"])

gulp.task('build-prod', ['clean-directory','webpack:build:prod', "server"])
...
gulp.task("server", function (callback) {
    ...
    app.use(express.static(myConfig.output.path))
...
gulp.task('clean-directory', function (callback) {
    htmlBuildPath = 'dist'
    return gulp.src(htmlBuildPath + '/*', {read: false})
        .pipe(clean())
